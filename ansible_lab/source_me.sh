...
export ANSIBLE_VAULT_PASSWORD_FILE=~/.ssh/szkolenie_vault.txt
...
if [ "$__venv_created" = true ] || [ "$__install_requirements" = true ]; then
  ...
  pip3 install -r $ANS_ROOT_DIR/requirements-azure.txt || (echo '!!! pip install failed' && return)
fi
